...
class PlaybackSession {
  ...
  /// Original AES-128 key URL per rendition — recorded by the variant handler
  /// so the /key route can proxy it to the player.
  final Map<String, String> keyUrlByRendition = {};

  /// Original upstream segment URLs per rendition, in playlist order.
  /// Index matches the `{n}` in `/seg/{rendition}/{n}.ts`.
  final Map<String, List<String>> segmentUrlsByRendition = {};

  /// Decrypted (or raw) AES-128 key bytes per rendition — populated when
  /// the /key route fetches and proxies the key.
  /// For DRM streams (is_drm=true), callers may populate this from drmKeys
  /// directly rather than waiting for a /key round-trip.
  final Map<String, Uint8List> keyBytesByRendition = {};

  /// AES-128 IV per rendition — parsed from EXT-X-KEY IV= attribute.
  /// Defaults to a zero-filled 16-byte value when not specified.
  final Map<String, Uint8List> ivByRendition = {};
}
